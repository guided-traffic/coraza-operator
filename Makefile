...
# PROTOC_INCLUDE is the include path for well-known proto types (google/protobuf).
PROTOC_INCLUDE ?= $(shell brew --prefix protobuf 2>/dev/null)/include

.PHONY: proto
proto: ## Regenerate Go stubs from proto/waf/v1/config.proto using protoc.
	...
	protoc \
		...
		--go-grpc_out=proto/waf/v1 \
		--go-grpc_opt=paths=source_relative \
		-I proto/waf/v1 \
		-I $(PROTOC_INCLUDE) \
		proto/waf/v1/config.proto
...
KIND_CLUSTER ?= coraza-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name coraza-operator-builder
	$(CONTAINER_TOOL) buildx use coraza-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm coraza-operator-builder
	rm Dockerfile.cross
